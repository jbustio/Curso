    ...
    def _unlink_except_state_new_cancel(self):
        for record in self:
            if record.state in ['new', 'canceled']:
                raise UserError(
                    f"Can't delete a property on state {record.state}")

        # if any(record.state in ['new', 'canceled'] for record in self):
        #     raise UserError(f"Can't delete an property on state New or Canceled ")
...
class EstatePropertyType(models.Model):
    ...
    def _compute_offer_count(self):
        for record in self:
            ...
            # count = record.offer_ids.search_count(
            #     [('property_type_id', '=', record.id)])
            count = len(record.offer_ids.ids)
...
class EstatePropertyOffer(models.Model):
    ...
    def create(self, vals):
        better_offers = self.env['estate.property.offer'].search_count(
            [('property_id', '=', vals['property_id']), ('price', '>=', vals['price'])])
        if better_offers:
            raise UserError(
                f"There are better offers for this property already")
